...
from sklearn.ensemble import IsolationForest as IsoFor
...
if __name__ == "__main__":
    ...
    print("Running isolation forest to filter out noise...")
    model = IsoFor().fit(np.reshape(cellImages,[-1,200*200]))
    joblib.dump(model, "./logs/model.pkl")
    predictions = model.predict(np.reshape(cellImages,[-1,200*200]))
    unique, counts = np.unique(predictions, return_counts=True)
    print(dict(zip(unique, counts)))
    blueCells = unique[np.argmax(counts)] # This is where the cells are likely to be
